...
def detect_nose(img, faceCascade):
    ...
    features = faceCascade.detectMultiScale(gray_img, 1.2, 3)
    nose_cords = []
    # drawing rectangle around the feature and labeling it
    for (x, y, w, h) in features:
        cv2.rectangle(img, (x,y), (x+w, y+h),  color['green'], 3)  #uncomment if you want to see face boundary
        cv2.circle(img, ((2*x+w)//2,(2*y+h)//2), 5, color['green'], 2)
...
while True:
    ...
    cv2.putText(img, cmd, (10,50), cv2.FONT_HERSHEY_SIMPLEX, 1, color['green'], 1, cv2.LINE_AA)
